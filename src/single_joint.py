...
class MultiGaitAdapter(Module):
    ...
    def __init__(self, in_features, initial_out_features=1):
    ...
    def forward(self, x):
        ...
        return self.prediction_head(out2)
    
def singlehead_training(net, nb_epochs, train_iters, criterion, optimizer, 
             val_dataloader=None, model_save_path='best_model.pth'):
    min_valid_loss = np.inf
    for epoch in range(nb_epochs):  # loop over the dataset multiple times
        train_loss = 0.0
        for X, y, t in tqdm(train_iters, desc=f'Epoch {epoch}'):       
        ...
        if val_dataloader is not None:
            valid_loss = 0.0
            net.eval()
            for X_val, y_val, t_val in val_dataloader:
                # Transfer Data to GPU if available
                if torch.cuda.is_available():
            ...
            print(f'Epoch {epoch+1} \t\t Training Loss: {train_loss / len(train_iters)} \t\t Validation Loss: {valid_loss}')
            ...
            print(f'Epoch {epoch+1} \t\t Training Loss: {train_loss / len(train_iters)}')
    
    print('Finished Training')
    return train_loss

def singlehead_validation(net, test_iters, criterion, optimizer):
    net.eval()
    valid_loss = 0.0
    actual = []
    predicted = []
    tasks = []
    #for i in tqdm(range(len(test_iters))):
...
if dataset == 'amputees':
    ...
    my_parser.add_argument(
        ...
        default='../data/amputees', 
        required=False)
    my_parser.add_argument(
        '--output', action='store', type=str, 
        default='../results/amputees_singlehead_joint', 
        required=False)
...
for subject_name in subjects:
    print(subject_name)
    model = MultiGaitAdapter(in_features = len(inputs))
    if torch.cuda.is_available():
        model = model.cuda()
    ...
    for i, train_task_id in enumerate(task_ids):
        print("Train task", train_task_id)
        subject_trainset = Replay_Data(
            ...
            ignore_backward=ignore_backward,
            time_delay=True,)
        print(len(subject_trainset))
        train_iters.append(subject_trainset)
        
    trainsets_merged = TaskBalancedDataset(train_iters)
        
    train_dataloader = DataLoader(trainsets_merged, shuffle=True, batch_size=100)
            
    train_loss = singlehead_training(
        model, nb_epochs, train_dataloader, criterion, optimizer, 
        val_size, model_save_path)
    # validation
    if val_size is not None:
        # validation on best model
        best_model = MultiGaitAdapter(in_features = len(inputs), n_heads=len(task_ids))
        best_model.load_state_dict(torch.load(model_save_path))   
    else:
        best_model = model
    if torch.cuda.is_available():
        best_model = best_model.cuda()
    valid_loss, actual, predicted, task_labels = singlehead_validation(
        best_model, test_dataloader, criterion, optimizer)
    ...
    valid_loss, actual, predicted, task_labels = singlehead_validation(
        model, test_dataloader, criterion, optimizer)
